...
class CPortalTurretTipController;
class CBeam;
class CSprite;
...
//-----------------------------------------------------------------------------
class CNPC_PortalFloorTurret : public CNPCBaseInteractive<CAI_BaseNPC>, public CDefaultPlayerPickupVPhysics
{
	DECLARE_CLASS(CNPC_PortalFloorTurret, CNPCBaseInteractive<CAI_BaseNPC>);
public:

	CNPC_PortalFloorTurret(void);
	...
	//added by pacca
	virtual void	Wake(inputdata_t &inputdata);
	virtual	bool	AllowedToIgnite(void) { return true; }	//turrets can always be ignited no matter what
...
protected:
	...
	CHandle<CPortalTurretTipController>	m_pMotionController;
...
class CPortalTurretTipController : public CPointEntity, public IMotionEvent
{
	DECLARE_CLASS(CPortalTurretTipController, CPointEntity);
	DECLARE_DATADESC();

public:

	~CPortalTurretTipController(void);
	void Spawn(void);
	void Activate(void);
	...
	static CPortalTurretTipController	*CreateTipController(CNPC_PortalFloorTurret *pOwner)
	{
		if (pOwner == NULL)
			return NULL;

		CPortalTurretTipController *pController = (CPortalTurretTipController *)Create("floorturret_tipcontroller", pOwner->GetAbsOrigin(), pOwner->GetAbsAngles());
...
private:
	...
	CNPC_PortalFloorTurret			*m_pParentTurret;
};

#endif //#ifndef NPC_PORTAL_TURRET_FLOOR_H
