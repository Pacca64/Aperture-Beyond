...
BEGIN_DATADESC( CTriggerPortal )
	...
	DEFINE_KEYFIELD( m_qFaceAngles, FIELD_VECTOR, "Direction" ),
	DEFINE_KEYFIELD(m_useDirection, FIELD_BOOLEAN, "UseDirection"),
...
//We have no client side code for this object!!!
/*
...
void CTriggerPortal::Touch( CBaseEntity *pOther )
{
	...
	if ( PassesTriggerFilters(pOther) )
	{
		...
		//Temporarily change our rotation
		if (m_useDirection){
			SetAbsAngles(m_qFaceAngles);
		}
		...
		//(Pacca) This code does NOT handle world collision! 
		//Seems to trigger 100% of the time, useless for dealing with player collision
		//My old code just pushed player in front of portal 100% of the time.

		// Teleportation caused us to hit something, deal with it.
		if ( tr.DidHit() )
		{
			/*
			DevMsg("Unfinished stuck in something statement triggered!");
			//pOther->Teleport(&ptNewOrigin, &qNewAngles, &vVelocity);

			Vector fwd, rt, up;
			AngleVectors(pPortal->GetAbsAngles(), &fwd, &rt, &up);

			Vector NewOriginForwards = ptNewOrigin + (fwd*100);

			pOther->Teleport(&NewOriginForwards, &qNewAngles, &vVelocity);
			DevMsg("Object pushed forwards");
			*/
		}

		if (!pOther->IsInWorld()){
			DevMsg("Portalled entity out of world!!! Plz fix");
		}

		//this line was added by me
		//revert modified rotation
		if (m_useDirection){
