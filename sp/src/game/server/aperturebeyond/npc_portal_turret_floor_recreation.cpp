...
#include "cbase.h"
#include "npc_portal_turret_floor_recreation.h"
#include "ai_senses.h"
#include "ai_memory.h"
...
//Debug visualization
ConVar	g_debug_portal_turret("g_debug_portal_turret", "0");
...
float CNPC_PortalFloorTurret::fMaxTipControllerVelocity = 300.0f * 300.0f;
float CNPC_PortalFloorTurret::fMaxTipControllerAngularVelocity = 90.0f * 90.0f;

#define	LASER_BEAM_SPRITE			"effects/redlaser1.vmt"
...
#define	FLOOR_TURRET_RANGE			1200 //originally 1200
...
//Datatable
BEGIN_DATADESC(CNPC_PortalFloorTurret)
...
DEFINE_KEYFIELD(m_gagged, FIELD_BOOLEAN, "Gagged"),
DEFINE_KEYFIELD(m_disableMotion, FIELD_BOOLEAN, "DisableMotion"),
DEFINE_FIELD(m_burnTalk, FIELD_BOOLEAN),
...
DEFINE_INPUTFUNC(FIELD_VOID, "Wake", Wake),
...
// Constructor
//-----------------------------------------------------------------------------
CNPC_PortalFloorTurret::CNPC_PortalFloorTurret(void) :
m_bActive(false),
m_hEyeGlow(NULL),
m_hLaser(NULL),
m_iAmmoType(-1),
...
void CNPC_PortalFloorTurret::Ignite(float flFlameLifetime, bool bNPCOnly, float flSize, bool bCalledByLevelDesigner)
{
	BaseClass::Ignite(flFlameLifetime, bNPCOnly, flSize, bCalledByLevelDesigner);
	EmitSound("NPC_FloorTurret.TalkStartBurning");
	InputSelfDestruct(inputdata_t());
}

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
Class_T	CNPC_PortalFloorTurret::Classify(void)
{
	if (m_bEnabled)	//just return none, so we can attack combine soldiers
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_PortalFloorTurret::UpdateOnRemove(void)
{
...
void CNPC_PortalFloorTurret::Precache(void)
{
	...
	// Activities
	ADD_CUSTOM_ACTIVITY(CNPC_PortalFloorTurret, ACT_FLOOR_TURRET_OPEN);
	ADD_CUSTOM_ACTIVITY(CNPC_PortalFloorTurret, ACT_FLOOR_TURRET_CLOSE);
	ADD_CUSTOM_ACTIVITY(CNPC_PortalFloorTurret, ACT_FLOOR_TURRET_CLOSED_IDLE);
	ADD_CUSTOM_ACTIVITY(CNPC_PortalFloorTurret, ACT_FLOOR_TURRET_OPEN_IDLE);
	ADD_CUSTOM_ACTIVITY(CNPC_PortalFloorTurret, ACT_FLOOR_TURRET_FIRE);
	...
	//portal 2 specific sounds
	PrecacheScriptSound("NPC_FloorTurret.TalkFlung");
	PrecacheScriptSound("NPC_FloorTurret.TalkBurned");
	PrecacheScriptSound("NPC_FloorTurret.TalkStartBurning");
	PrecacheScriptSound("NPC_FloorTurret.Distressed");
...
void CNPC_PortalFloorTurret::Spawn(void)
{
	...
	m_burnTalk = false;	//true after we've talked before exploding
	...
	//disable HL2 turret "alarm" sounds. These are the pinging noises HL2 turrets make; portal turrets don't do that!
	...
	{
		SetThink(&CNPC_PortalFloorTurret::AutoSearchThink);
		SetEyeState(TURRET_EYE_DORMANT);
	}
	else
	{
		SetThink(&CNPC_PortalFloorTurret::DisabledThink);
	...
	SetUse(&CNPC_PortalFloorTurret::ToggleUse);
	...
	//Added to test for adding portal 2 features. In this case, disable motion
	//SetMoveType(MOVETYPE_VPHYSICS);	//used by the turret, self explanatory
	if (m_disableMotion){
		SetMoveType(MOVETYPE_NONE);	//disables physics
	}

	/*
	CBaseAnimating *pBeamEnt = this;

	CBeam *laser = CBeam::BeamCreate(
		LASER_BEAM_SPRITE, 1.0f);
	laser->EntsInit(pBeamEnt, pBeamEnt);

	int	startAttachment = LookupAttachment("eyes");
	int endAttachment = 1;

	laser->FollowEntity(pBeamEnt);

	laser->AddSpawnFlags(SF_BEAM_STARTON);
	laser->SetStartAttachment(startAttachment);
	laser->SetEndAttachment(endAttachment);
	laser->SetNoise(random->RandomFloat(8.0f, 16.0f));
	laser->SetColor(255, 255, 255);
	laser->SetScrollRate(25);
	laser->SetBrightness(128);
	laser->SetWidth(0);
	laser->SetEndWidth(random->RandomFloat(2, 4));
	*/
}
...
void CNPC_PortalFloorTurret::Activate(void)
{
	...
	if (!m_pMotionController)
	{
		...
		m_pMotionController = CPortalTurretTipController::CreateTipController(this);
...
bool CNPC_PortalFloorTurret::CreateVPhysics(void)
{
	//Spawn our physics hull
	if (VPhysicsInitNormal(SOLID_VPHYSICS, 0, false) == NULL)
	{
		DevMsg("npc_portal_turret_floor unable to spawn physics object!\n");
	}
...
//-----------------------------------------------------------------------------
void CNPC_PortalFloorTurret::Retire(void)
{
...
//-----------------------------------------------------------------------------
void CNPC_PortalFloorTurret::Deploy(void)
{
	...
	if (IsActivityFinished())
	{
		...
		m_flPlaybackRate = 0;
		SetThink(&CNPC_PortalFloorTurret::SearchThink);
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_PortalFloorTurret::OnPhysGunDrop(CBasePlayer *pPhysGunUser, PhysGunDrop_t Reason)
{
	m_hPhysicsAttacker = pPhysGunUser;
	m_flLastPhysicsInfluenceTime = gpGlobals->curtime;
...
//-----------------------------------------------------------------------------
bool CNPC_PortalFloorTurret::HasPreferredCarryAnglesForPlayer(CBasePlayer *pPlayer)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CNPC_PortalFloorTurret::OnAttemptPhysGunPickup(CBasePlayer *pPhysGunUser, PhysGunPickup_t reason)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CNPC_PortalFloorTurret::HandleInteraction(int interactionType, void *data, CBaseCombatCharacter *sourceEnt)
{
...
//-----------------------------------------------------------------------------
float CNPC_PortalFloorTurret::MaxYawSpeed(void)
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_PortalFloorTurret::WasJustDroppedByPlayer(void)
{
...
//-----------------------------------------------------------------------------
bool CNPC_PortalFloorTurret::UpdateFacing(void)
{
	bool  bMoved = false;
	UpdateMuzzleMatrix();
	...
	if (g_debug_portal_turret.GetBool())
	{
...
void CNPC_PortalFloorTurret::DryFire(void)
{
...
//-----------------------------------------------------------------------------
void CNPC_PortalFloorTurret::SuppressThink(void)
{
	...
	if (!GetEnemy())
	{
		SetThink(&CNPC_PortalFloorTurret::ActiveThink);
		return;
	}
...
//-----------------------------------------------------------------------------
void CNPC_PortalFloorTurret::ActiveThink(void)
{
	//Allow descended classes a chance to do something before the think function
	...
	if ((bEnemyVisible == false) || (flDistToEnemy > FLOOR_TURRET_RANGE))
	{
		...
		ClearEnemyMemory();
		SetEnemy(NULL);
		SetThink(&CNPC_PortalFloorTurret::SuppressThink);
...
//-----------------------------------------------------------------------------
void CNPC_PortalFloorTurret::SearchThink(void)
{
	//Allow descended classes a chance to do something before the think function
	...
	if (gpGlobals->curtime > m_flLastSight)
	{
		...
		m_flLastSight = 0;
		SetThink(&CNPC_PortalFloorTurret::Retire);
		return;
	}

	//Display that we're scanning
	m_vecGoalAngles.x = GetAbsAngles().x + (sin(gpGlobals->curtime * 1.0f) * 15.0f);
	m_vecGoalAngles.y = GetAbsAngles().y + (sin(gpGlobals->curtime * 2.0f) * 37.5f);// * 60.0f);
...
//-----------------------------------------------------------------------------
void CNPC_PortalFloorTurret::AutoSearchThink(void)
{
	//Allow descended classes a chance to do something before the think function
	...
	if (GetEnemy() != NULL)
	{
		SetThink(&CNPC_PortalFloorTurret::Deploy);
		if (!m_bNoAlarmSounds)
		{
...
// Purpose: Fire!
//-----------------------------------------------------------------------------
void CNPC_PortalFloorTurret::Shoot(const Vector &vecSrc, const Vector &vecDirToEnemy, bool bStrict)
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_PortalFloorTurret::IsValidEnemy(CBaseEntity *pEnemy)
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_PortalFloorTurret::CanBeAnEnemyOf(CBaseEntity *pEnemy)
{
	...
	else
	{
		...
		else if (IsActivityFinished())
		{
			...
			if (UpdateFacing() == false)
			{
				...
				SetThink(&CNPC_PortalFloorTurret::InactiveThink);
...
//-----------------------------------------------------------------------------
void CNPC_PortalFloorTurret::InactiveThink(void)
{
	// Update our PVS state
	CheckPVSCondition();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_PortalFloorTurret::ReturnToLife(void)
{
	m_flThrashTime = 0;
...
//-----------------------------------------------------------------------------
void CNPC_PortalFloorTurret::DisabledThink(void)
{
...
//-----------------------------------------------------------------------------
void CNPC_PortalFloorTurret::HackFindEnemy(void)
{
...
//-----------------------------------------------------------------------------
inline bool CNPC_PortalFloorTurret::OnSide(void)
{
	Vector	up;
...
bool CNPC_PortalFloorTurret::PreThink(turretState_e state)
{
	...
	if (!IsBeingCarriedByPlayer())
	{
		...
		{
			//I think disabling thrashing here might be the cause of the "out of ammo turrets lock up" bug.
			//disabling the no thrashing code may hopefully restore defective turrets.
			if (true)//HasSpawnFlags(SF_FLOOR_TURRET_OUT_OF_AMMO) == false)
			{
				if (!m_gagged){
				...
				SetThink(&CNPC_PortalFloorTurret::TippedThink);
				SetEyeState(TURRET_EYE_SEE_TARGET);
...
//-----------------------------------------------------------------------------
void CNPC_PortalFloorTurret::SetEyeState(eyeState_t state)
{
	...
	if (m_hLaser == NULL)
	{
		//get a forward facing vector from our angles
		Vector vecForward;
		AngleVectors(this->GetAbsAngles(), &vecForward); //Assign vecForward a vector facing the EyeAngles

		//Create a target, parent it to ourselves, then set it's parent attachment to our eyes.
		//Makes a target for the laser to follow, just like in gmod!
		CBaseEntity *target = CBaseEntity::Create("info_target", this->GetAbsOrigin() + vecForward * 5000, this->GetAbsAngles(), this);
		target->SetParent(this);
		target->SetParentAttachment("SetParentAttachment", "eyes", true);

		m_hLaser = CBeam::BeamCreate(LASER_BEAM_SPRITE, 0.2f);
		if (m_hLaser == NULL)
			return;

		m_hLaser->EntsInit(target, this);
		...
		m_hLaser->AddSpawnFlags(SF_BEAM_STARTON);
		m_hLaser->SetStartAttachment(startAttachment);
		m_hLaser->SetEndAttachment(endAttachment);
		m_hLaser->SetNoise(0);
		m_hLaser->SetColor(255, 255, 255);
		m_hLaser->SetScrollRate(0);
		m_hLaser->SetBrightness(255);
		m_hLaser->SetWidth(0.75);
		m_hLaser->SetEndWidth(0.75);
	}
	...
	//removed citizen turret check; we always want lasers!
	/*
	if (m_hLaser == NULL)
	{
...
//-----------------------------------------------------------------------------
void CNPC_PortalFloorTurret::Ping(void)
{
...
//-----------------------------------------------------------------------------
void CNPC_PortalFloorTurret::Toggle(void)
{
...
//-----------------------------------------------------------------------------
void CNPC_PortalFloorTurret::Enable(void)
{
...
//-----------------------------------------------------------------------------
void CNPC_PortalFloorTurret::Disable(void)
{
	//This turret is on its side, it can't function
...
//-----------------------------------------------------------------------------
void CNPC_PortalFloorTurret::InputToggle(inputdata_t &inputdata)
{
	Toggle();
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_PortalFloorTurret::InputEnable(inputdata_t &inputdata)
{
	Enable();
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_PortalFloorTurret::InputDisable(inputdata_t &inputdata)
{
	Disable();
}
...
//-----------------------------------------------------------------------------
void CNPC_PortalFloorTurret::InputDepleteAmmo(inputdata_t &inputdata)
{
...
//-----------------------------------------------------------------------------
void CNPC_PortalFloorTurret::InputRestoreAmmo(inputdata_t &inputdata)
{
...
//-----------------------------------------------------------------------------
void CNPC_PortalFloorTurret::ToggleUse(CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value)
{
	switch (useType)
	{
	case USE_OFF:
...
//-----------------------------------------------------------------------------
int CNPC_PortalFloorTurret::VPhysicsTakeDamage(const CTakeDamageInfo &info)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_PortalFloorTurret::SpinUp(void)
{
}
...
// Output : const QAngle
//-----------------------------------------------------------------------------
QAngle CNPC_PortalFloorTurret::PreferredCarryAngles(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_PortalFloorTurret::SpinDown(void)
{
}
...
// Output : float
//-----------------------------------------------------------------------------
float CNPC_PortalFloorTurret::GetAttackDamageScale(CBaseEntity *pVictim)
{
...
//------------------------------------------------------------------------------
CBasePlayer *CNPC_PortalFloorTurret::HasPhysicsAttacker(float dt)
{
...
//-----------------------------------------------------------------------------
int CNPC_PortalFloorTurret::DrawDebugTextOverlays(void)
{
...
void CNPC_PortalFloorTurret::UpdateMuzzleMatrix()
{
...
//			reappear.
//-----------------------------------------------------------------------------
void CNPC_PortalFloorTurret::PlayerPenetratingVPhysics(void)
{
	// We don't care!
}

#define SELF_DESTRUCT_DURATION			2.0f
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_PortalFloorTurret::BreakThink(void)
{
	Vector vecUp;
...
//-----------------------------------------------------------------------------
void CNPC_PortalFloorTurret::SelfDestructThink(void)
{
	// Continue to animate
	PreThink(TURRET_SELF_DESTRUCTING);

	//check if we should burn talk
	//happens halfway before exploding, and disables burn talking once triggered
	if (!m_burnTalk && (gpGlobals->curtime - m_flDestructStartTime) >= (SELF_DESTRUCT_DURATION/2)){
		EmitSound("NPC_FloorTurret.TalkBurned");
		m_burnTalk = true;
	}
	...
	{
		SetThink(&CNPC_PortalFloorTurret::BreakThink);
		SetNextThink(gpGlobals->curtime + 0.1f);
...
//-----------------------------------------------------------------------------
void CNPC_PortalFloorTurret::InputSelfDestruct(inputdata_t &inputdata)
{
	// Ka-boom!
	...
	SetThink(&CNPC_PortalFloorTurret::SelfDestructThink);
	SetNextThink(gpGlobals->curtime + 0.1f);
...
void CNPC_PortalFloorTurret::Wake(inputdata_t &inputdata){
	BaseClass::Wake();
	IPhysicsObject *pPhys = this->VPhysicsGetObject();
	pPhys->Wake();
	DevMsg("Im awake lol");
}
...
LINK_ENTITY_TO_CLASS(portalfloorturret_tipcontroller, CPortalTurretTipController);


//---------------------------------------------------------
// Save/Restore
//---------------------------------------------------------
BEGIN_DATADESC(CPortalTurretTipController)
...
CPortalTurretTipController::~CPortalTurretTipController()
{
	if (m_pController)
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CPortalTurretTipController::Spawn(void)
{
	m_bEnabled = true;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CPortalTurretTipController::Activate(void)
{
	BaseClass::Activate();
...
//-----------------------------------------------------------------------------
IMotionEvent::simresult_e CPortalTurretTipController::Simulate(IPhysicsMotionController *pController, IPhysicsObject *pObject, float deltaTime, Vector &linear, AngularImpulse &angular)
{
	if (Enabled() == false)
		return SIM_NOTHING;
	...
	else
	{
		...
		if ((vecVelocity.LengthSqr() > CNPC_PortalFloorTurret::fMaxTipControllerVelocity) || (angImpulse.LengthSqr() > CNPC_PortalFloorTurret::fMaxTipControllerAngularVelocity))
			return SIM_NOTHING;
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CPortalTurretTipController::Enable(bool state)
{
	m_bEnabled = state;
}
...
// Input  : time - 
//-----------------------------------------------------------------------------
void CPortalTurretTipController::Suspend(float time)
{
	m_flSuspendTime = gpGlobals->curtime + time;
}


float CPortalTurretTipController::SuspendedTill(void)
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CPortalTurretTipController::Enabled(void)
{
...
//
//-----------------------------------------------------------------------------
AI_BEGIN_CUSTOM_NPC(npc_portal_turret_floor, CNPC_PortalFloorTurret)
